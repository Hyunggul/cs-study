...
- 예를 들어, **http**://www.joes-hardware.com/seasonal/index-fall.html 이라는 URL을 불러오고 싶다면

  1. http는 URL의 스킴이며 `스킴은 웹 클라이언트가 리소스에 어떻게 접근하는지 알려준다.` 해당 경우에는 HTTP 프로토콜을 사용한다는 뜻이다.
  
  2. www.joes-hardware.com은 서버의 위치이다. 이는 웹 클라이언트가 `리소스가 어디에 호스팅 되어 있는지 알려준다.`
  
  3. /seaonal/index-fall.html은 리소스의 경로이다. `서버에 존재하는 리소스 중 요청받은 리소스가 무엇인지 알려준다.`

<br>

- URL은 http 뿐만 아니라 **ftp**://ftf.lots-o.books.com, rtsp://www.joes-hardware.com:554/cto_video 등 인터넷에 있는 어떤 리소스든지 가리킬 수 있다.
...
- 예를 들어, **http**://www.joes-hardware.com/hammers;sale=false/index.html;grphics=true에 대해 설명하자면
...
- ./hammers.html 이라고 적혀 있는 경우
    ...
    3. 이를 합치면 **http**://www.joes.hardware.com/hammers.html
...
- 예를 들어 빈 문자는 **http**://www.joes-hardware.com/more%20tools.html와 같이 표현한다.
