...
- **퀵 소트**:
  - **평균 시간 복잡도**: \(O(n \log n)\)
  - **최악 시간 복잡도**: \(O(n^2)\)
  - **특징**: 분할 정복, In-place, 피벗 선택 중요
  - **장점**: 평균적으로 빠름, In-place 정렬
  - **단점**: 최악의 경우 성능 저하

- **힙 소트**:
  - **평균 및 최악 시간 복잡도**: \(O(n \log n)\)
  - **특징**: 완전 이진 트리, 최대 힙 이용
  - **장점**: 시간 복잡도 안정적, In-place 정렬
  - **단점**: 일반적으로 퀵 소트보다 느림, 캐시 비효율성

두 알고리즘 모두 각각의 장단점이 있으며, 사용하려는 데이터와 상황에 따라 적절한 알고리즘을 선택하는 것이 중요합니다.
</div>
</details>
<br>
...
데이터 파일을 전혀 읽지 않고 인덱스 읽어서 쿼리를 모두 처리할 수 있을 때 표시

인덱스를 이용해 처리하는 쿼리에서 가장 큰 부하를 차지하는 부분은 인덱스 검색에서 일치하는 키 값들의 레코드를 읽기 위해 데이터 파일을 검색하는 작업

InnoDB의 클러스터링 인덱스 특성 상 쿼리가 커버링 인덱스로 처리될 가능성이 높음

### Using index condition

컨디션 푸시 다운 최적화를 사용하면 Using index condition 표시

### Using index for group by

group by 처리를 위해 MySQL 서버는 그루핑 기준 칼럼을 이용해 정렬 작업을 수행하고 다시 정렬된 결과를 그루핑하는 형태의 고부하 작업을 필요로 함

근데 인덱스를 이용하면? 별도 추가 정렬 작업 없이 그루핑 작업만 수행

### Using index for skip scan

루스 인덱스 스캔 최적화를 확장한 인덱스 스킵 스캔 최적화가 도입

### Using join buffer

드리븐 테이블은 검색 위주로 사용되기 때문에 인덱스가 없으면 성능에 미치는 영향이 매우 큼

드리븐 테이블에 검색을 위한 적절 인덱스가 없으면 해시 조인 사용

### Using MRR

MySQL 엔진은 실행 계획을 수립하고 실행 계획에 맞게 스토리지 엔진의 API를 호출해서 쿼리를 최적화

그래서 스토리지 엔진 레벨에서는 쿼리 실행의 전체적인 부분을 알지 못해서 최적화에 한계

많은 레코드를 읽는 과정이더라도 레코드를 한 건 한 건 읽어서 반환하는 방식으로밖에 작동하지 않음

실제 매번 읽어서 반환하는 레코드가 동일 페이지에 있더라도 레코드 단위로 API 호출이 필요한 것

이런 단점을 해결하기 위해 MRR 최적화 도입
...
MySQL 엔진 레이어에서 별도의 가공을 해서 필터링 작업을 처리한 경우에만 Extra 칼럼에 Using where 코멘트

<p align="center"><img src="./images/10_84.png" width="80%"></p>

<br>

만약에 스토리지 엔진에서 200건을 읽었는데 별도 가공 없이 그대로 클라이언트에게 전달하면 Using where가 표시되지 않음
