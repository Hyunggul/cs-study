...
> 8.0.18 버전부터 해시 조인 알고리즘이 도입됐고, 8.0.20 버전부터 BNL 조인이 사>용되지 않고 해시 조인 알고리즘으로 대체
...
last_name='Action' 조건으로 인덱스 레인지 스캔을 하고 테이블의 레코드를 읽은 후 first_name LIKE '%sal' 조건에 부합되는지 여부 비교

만약 last_name='Action' 조건과 일치하는 레코드가 10만 건인데 그 중 1건만 first_name LIKE '%sal' 조건에 맞는다면 99,999건의 레코드 읽기가 불필요
